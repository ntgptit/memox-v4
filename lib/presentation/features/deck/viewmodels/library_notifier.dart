...
final libraryNotifierProvider =
    AsyncNotifierProvider.autoDispose<LibraryNotifier, List<DeckNode>>(
      LibraryNotifier.new,
    );

class LibraryNotifier extends AutoDisposeAsyncNotifier<List<DeckNode>> {
  ...
  Future<void> createDeck({int? parentDeckId, required String name}) async {
    final pairId = ref
        .read(languagePairNotifierProvider)
        .valueOrNull
        ?.active
        ?.id;
