...
# PROMPT ID: MX-KIT-TO-FLUTTER-drawer

Repo:    https://github.com/ntgptit/memox-v4
Base:    main
Screen:  `drawer`   ·   feature: `(resolve manually — no keyed FE file found)`   ·   FE file: `(resolve manually)`
States:  `open` · `add-language` · `remove-language`

## Goal
Chuyển/khớp màn `drawer` từ UI kit (JSX) sang Flutter cho ĐÚNG kit — verify-gated, không đoán.
...
Kit truth:
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/drawer.md`     — DOM resolve token (node/layout/font/color/bbox/mx: hint)
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/drawer--*.png` — ảnh light+dark từng state (ĐỌC ẢNH, không đọc JSX kit)
Contracts (skeleton đã sinh sẵn):
- `tool/parity/contracts/drawer.gen.json`                 — IDENTITY: key + component + variant
- `tool/parity/contracts/drawer.slots.skeleton.json`     — ĐỀ XUẤT text→MxTextRole + l10n/bind (curate)
- `tool/parity/contracts/drawer.states.skeleton.json`    — ĐỀ XUẤT node theo state (curate)
...
1. Đọc `drawer.md` + shots. Mỗi state: xác định node xuất hiện, layout (spec `layout:` → Row/Column/Wrap/Grid, KHÔNG toạ độ tuyệt đối), style (token `--memox-*` → `mx_*.dart`).
2. Curate `tool/parity/contracts/drawer.slots.json` TỪ skeleton: VERIFY `role` (sửa nhầm label↔body↔title); điền `l10n`=ARB key (copy tĩnh) hoặc `bind`=domain field; bỏ chrome. Format theo `dashboard.slots.json`.
3. Curate `tool/parity/contracts/drawer.states.json` TỪ skeleton: giữ node BODY từng state (bỏ chrome), key `mx-node:` prefix; state kit chưa render riêng ở FE → giữ + ghi "chưa map" (coverage gap). Theo `review.states.json`.
...
6. Test `test/presentation/features/(resolve manually — no keyed FE file found)/drawer_parity_test.dart` — COPY `review_parity_test.dart`, đổi screen id + state-map (cách seed/pump tới từng state). Assert mỗi keyed MxCard: present-in-state → render + variant + slot role (+ l10n string nếu slot l10n); absent-in-state → `findsNothing`.
7. Xoá skeleton đã curate: `drawer.slots.skeleton.json` + `drawer.states.skeleton.json`.
...
- `git commit -m "feat((resolve manually — no keyed FE file found)): convert drawer to Flutter — identity + slots + per-state parity"` → commit WBS → `git push origin claude/kit-to-flutter-drawer` → xác nhận local==remote.
