...
# PROMPT ID: MX-KIT-TO-FLUTTER-export

Repo:    https://github.com/ntgptit/memox-v4
Base:    main
Screen:  `export`   ·   feature: `import_export`   ·   FE file: `import_export/screens/export_screen.dart`
States:  `config` · `exporting` · `done`

## Goal
Chuyển/khớp màn `export` từ UI kit (JSX) sang Flutter cho ĐÚNG kit — verify-gated, không đoán.
...
Kit truth:
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/export.md`     — DOM resolve token (node/layout/font/color/bbox/mx: hint)
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/export--*.png` — ảnh light+dark từng state (ĐỌC ẢNH, không đọc JSX kit)
Contracts (skeleton đã sinh sẵn):
- `tool/parity/contracts/export.gen.json`                 — IDENTITY: key + component + variant
- `tool/parity/contracts/export.slots.skeleton.json`     — ĐỀ XUẤT text→MxTextRole + l10n/bind (curate)
- `tool/parity/contracts/export.states.skeleton.json`    — ĐỀ XUẤT node theo state (curate)
...
- FE hiện tại của màn: `lib/presentation/features/import_export/screens/export_screen.dart`
...
1. Đọc `export.md` + shots. Mỗi state: xác định node xuất hiện, layout (spec `layout:` → Row/Column/Wrap/Grid, KHÔNG toạ độ tuyệt đối), style (token `--memox-*` → `mx_*.dart`).
2. Curate `tool/parity/contracts/export.slots.json` TỪ skeleton: VERIFY `role` (sửa nhầm label↔body↔title); điền `l10n`=ARB key (copy tĩnh) hoặc `bind`=domain field; bỏ chrome. Format theo `dashboard.slots.json`.
3. Curate `tool/parity/contracts/export.states.json` TỪ skeleton: giữ node BODY từng state (bỏ chrome), key `mx-node:` prefix; state kit chưa render riêng ở FE → giữ + ghi "chưa map" (coverage gap). Theo `review.states.json`.
4. Implement/align `lib/presentation/features/import_export/screens/export_screen.dart`: CHỈ Mx* + token (KHÔNG raw color/spacing/radius/text-style/duration, KHÔNG hardcode string). Mỗi node trong `.gen.json` → widget + `key: ValueKey('mx-node:<id>')` + variant khớp. Mỗi slot → `MxText` role đúng (l10n qua AppLocalizations, bind qua provider). State drive từ provider. Layering UseCase→Repository(interface)→DataSource; presentation không import data.
5. l10n: thêm key ngữ nghĩa vào CẢ `app_en.arb` + `app_vi.arb`.
6. Test `test/presentation/features/import_export/export_parity_test.dart` — COPY `review_parity_test.dart`, đổi screen id + state-map (cách seed/pump tới từng state). Assert mỗi keyed MxCard: present-in-state → render + variant + slot role (+ l10n string nếu slot l10n); absent-in-state → `findsNothing`.
7. Xoá skeleton đã curate: `export.slots.skeleton.json` + `export.states.skeleton.json`.
...
- `git commit -m "feat(import_export): convert export to Flutter — identity + slots + per-state parity"` → commit WBS → `git push origin claude/kit-to-flutter-export` → xác nhận local==remote.
