...
# PROMPT ID: MX-KIT-TO-FLUTTER-flashcard-editor

Repo:    https://github.com/ntgptit/memox-v4
Base:    main
Screen:  `flashcard-editor`   ·   feature: `flashcard`   ·   FE file: `flashcard/screens/flashcard_editor_screen.dart`
States:  `create` · `edit` · `validation` · `duplicate` · `multi-meaning` · `audio`

## Goal
Chuyển/khớp màn `flashcard-editor` từ UI kit (JSX) sang Flutter cho ĐÚNG kit — verify-gated, không đoán.
...
Kit truth:
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/flashcard-editor.md`     — DOM resolve token (node/layout/font/color/bbox/mx: hint)
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/flashcard-editor--*.png` — ảnh light+dark từng state (ĐỌC ẢNH, không đọc JSX kit)
Contracts (skeleton đã sinh sẵn):
- `tool/parity/contracts/flashcard-editor.gen.json`                 — IDENTITY: key + component + variant
- `tool/parity/contracts/flashcard-editor.slots.skeleton.json`     — ĐỀ XUẤT text→MxTextRole + l10n/bind (curate)
- `tool/parity/contracts/flashcard-editor.states.skeleton.json`    — ĐỀ XUẤT node theo state (curate)
...
- FE hiện tại của màn: `lib/presentation/features/flashcard/screens/flashcard_editor_screen.dart`
...
1. Đọc `flashcard-editor.md` + shots. Mỗi state: xác định node xuất hiện, layout (spec `layout:` → Row/Column/Wrap/Grid, KHÔNG toạ độ tuyệt đối), style (token `--memox-*` → `mx_*.dart`).
2. Curate `tool/parity/contracts/flashcard-editor.slots.json` TỪ skeleton: VERIFY `role` (sửa nhầm label↔body↔title); điền `l10n`=ARB key (copy tĩnh) hoặc `bind`=domain field; bỏ chrome. Format theo `dashboard.slots.json`.
3. Curate `tool/parity/contracts/flashcard-editor.states.json` TỪ skeleton: giữ node BODY từng state (bỏ chrome), key `mx-node:` prefix; state kit chưa render riêng ở FE → giữ + ghi "chưa map" (coverage gap). Theo `review.states.json`.
4. Implement/align `lib/presentation/features/flashcard/screens/flashcard_editor_screen.dart`: CHỈ Mx* + token (KHÔNG raw color/spacing/radius/text-style/duration, KHÔNG hardcode string). Mỗi node trong `.gen.json` → widget + `key: ValueKey('mx-node:<id>')` + variant khớp. Mỗi slot → `MxText` role đúng (l10n qua AppLocalizations, bind qua provider). State drive từ provider. Layering UseCase→Repository(interface)→DataSource; presentation không import data.
5. l10n: thêm key ngữ nghĩa vào CẢ `app_en.arb` + `app_vi.arb`.
6. Test `test/presentation/features/flashcard/flashcard_editor_parity_test.dart` — COPY `review_parity_test.dart`, đổi screen id + state-map (cách seed/pump tới từng state). Assert mỗi keyed MxCard: present-in-state → render + variant + slot role (+ l10n string nếu slot l10n); absent-in-state → `findsNothing`.
7. Xoá skeleton đã curate: `flashcard-editor.slots.skeleton.json` + `flashcard-editor.states.skeleton.json`.
...
- `git commit -m "feat(flashcard): convert flashcard-editor to Flutter — identity + slots + per-state parity"` → commit WBS → `git push origin claude/kit-to-flutter-flashcard-editor` → xác nhận local==remote.
