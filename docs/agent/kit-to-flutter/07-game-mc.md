...
# PROMPT ID: MX-KIT-TO-FLUTTER-game-mc

Repo:    https://github.com/ntgptit/memox-v4
Base:    main
Screen:  `game-mc`   ·   feature: `game`   ·   FE file: `game/screens/game_screen.dart`
States:  `waiting` · `correct` · `wrong` · `complete`

## Goal
Chuyển/khớp màn `game-mc` từ UI kit (JSX) sang Flutter cho ĐÚNG kit — verify-gated, không đoán.
...
Kit truth:
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/game-mc.md`     — DOM resolve token (node/layout/font/color/bbox/mx: hint)
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/game-mc--*.png` — ảnh light+dark từng state (ĐỌC ẢNH, không đọc JSX kit)
Contracts (skeleton đã sinh sẵn):
- `tool/parity/contracts/game-mc.gen.json`                 — IDENTITY: key + component + variant
- `tool/parity/contracts/game-mc.slots.skeleton.json`     — ĐỀ XUẤT text→MxTextRole + l10n/bind (curate)
- `tool/parity/contracts/game-mc.states.skeleton.json`    — ĐỀ XUẤT node theo state (curate)
...
1. Đọc `game-mc.md` + shots. Mỗi state: xác định node xuất hiện, layout (spec `layout:` → Row/Column/Wrap/Grid, KHÔNG toạ độ tuyệt đối), style (token `--memox-*` → `mx_*.dart`).
2. Curate `tool/parity/contracts/game-mc.slots.json` TỪ skeleton: VERIFY `role` (sửa nhầm label↔body↔title); điền `l10n`=ARB key (copy tĩnh) hoặc `bind`=domain field; bỏ chrome. Format theo `dashboard.slots.json`.
3. Curate `tool/parity/contracts/game-mc.states.json` TỪ skeleton: giữ node BODY từng state (bỏ chrome), key `mx-node:` prefix; state kit chưa render riêng ở FE → giữ + ghi "chưa map" (coverage gap). Theo `review.states.json`.
...
6. Test `test/presentation/features/game/game_mc_parity_test.dart` — COPY `review_parity_test.dart`, đổi screen id + state-map (cách seed/pump tới từng state). Assert mỗi keyed MxCard: present-in-state → render + variant + slot role (+ l10n string nếu slot l10n); absent-in-state → `findsNothing`.
7. Xoá skeleton đã curate: `game-mc.slots.skeleton.json` + `game-mc.states.skeleton.json`.
...
- `git commit -m "feat(game): convert game-mc to Flutter — identity + slots + per-state parity"` → commit WBS → `git push origin claude/kit-to-flutter-game-mc` → xác nhận local==remote.
