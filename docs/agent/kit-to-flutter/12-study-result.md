...
# PROMPT ID: MX-KIT-TO-FLUTTER-study-result

Repo:    https://github.com/ntgptit/memox-v4
Base:    main
Screen:  `study-result`   ·   feature: `study`   ·   FE file: `study/screens/study_session_screen.dart`
States:  `standard` · `goal-met` · `goal-missed` · `many-wrong` · `finalizing` · `retry-finalize` · `finalize-error`

## Goal
Chuyển/khớp màn `study-result` từ UI kit (JSX) sang Flutter cho ĐÚNG kit — verify-gated, không đoán.
...
Kit truth:
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/study-result.md`     — DOM resolve token (node/layout/font/color/bbox/mx: hint)
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/study-result--*.png` — ảnh light+dark từng state (ĐỌC ẢNH, không đọc JSX kit)
Contracts (skeleton đã sinh sẵn):
- `tool/parity/contracts/study-result.gen.json`                 — IDENTITY: key + component + variant
- `tool/parity/contracts/study-result.slots.skeleton.json`     — ĐỀ XUẤT text→MxTextRole + l10n/bind (curate)
- `tool/parity/contracts/study-result.states.skeleton.json`    — ĐỀ XUẤT node theo state (curate)
...
1. Đọc `study-result.md` + shots. Mỗi state: xác định node xuất hiện, layout (spec `layout:` → Row/Column/Wrap/Grid, KHÔNG toạ độ tuyệt đối), style (token `--memox-*` → `mx_*.dart`).
2. Curate `tool/parity/contracts/study-result.slots.json` TỪ skeleton: VERIFY `role` (sửa nhầm label↔body↔title); điền `l10n`=ARB key (copy tĩnh) hoặc `bind`=domain field; bỏ chrome. Format theo `dashboard.slots.json`.
3. Curate `tool/parity/contracts/study-result.states.json` TỪ skeleton: giữ node BODY từng state (bỏ chrome), key `mx-node:` prefix; state kit chưa render riêng ở FE → giữ + ghi "chưa map" (coverage gap). Theo `review.states.json`.
...
6. Test `test/presentation/features/study/study_result_parity_test.dart` — COPY `review_parity_test.dart`, đổi screen id + state-map (cách seed/pump tới từng state). Assert mỗi keyed MxCard: present-in-state → render + variant + slot role (+ l10n string nếu slot l10n); absent-in-state → `findsNothing`.
7. Xoá skeleton đã curate: `study-result.slots.skeleton.json` + `study-result.states.skeleton.json`.
...
- `git commit -m "feat(study): convert study-result to Flutter — identity + slots + per-state parity"` → commit WBS → `git push origin claude/kit-to-flutter-study-result` → xác nhận local==remote.
