# V.2 — Domain test sweep (SRS invariants)
...
>
> Size **M** · Deps **DM.4** · Branch `build/v2`


## Goal

Edge cases + scheduler invariants for the SRS engine and study use cases.
...
2. Push `build/v2`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `V.2` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.