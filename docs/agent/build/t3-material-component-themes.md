# T.3 — Material component themes

> **Loop task** (theme / UI foundation). Self-contained — execute fully in one iteration, then tick `T.3` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **T.1** · Branch `build/t3`


## Goal

Map tokens onto Material component themes so stock widgets inherit the kit look.
...
1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/t3`.
...
- Button/Card/AppBar/NavigationBar/Chip/Switch/Dialog/Input. Stock FilledButton must match kit primary.
...
2. Push `build/t3`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `T.3` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.