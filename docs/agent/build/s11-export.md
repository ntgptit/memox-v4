...
>
> Size **M** · Deps **Phase K,H + DM.7, DM.8** · Branch `build/s11`


## Goal

Build the **export** screen + its 2 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.7, DM.8** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/export/Export.jsx` — screen composition (components, states, state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/export.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/export--*--{light,dark}.png` — visual reference per state.
...
- `lib/presentation/features/export/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
...
2. Read `Export.jsx` → enumerate **states** (screen + `specs/export.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s11`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.11` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.