...
>
> Size **M** · Deps **Phase K + DM.7** · Branch `build/s11`


## Goal

Build the **export** screen + its 2 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.7** use-case state via `@riverpod` providers, matching the kit for every state.
