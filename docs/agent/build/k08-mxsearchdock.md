...
> **Loop task** (composite component). Self-contained — execute fully in one iteration, then tick `K.08` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S–L** · Deps **Phase T** · Branch `build/k08`


## Goal

Build the Flutter widget **MxSearchDock** mirroring the kit component, driven entirely by design tokens + `MxTheme` (composite layer).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/navigation/MxSearchDock.d.ts` — typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/navigation/MxSearchDock.prompt.md` — intent + JSX usage examples.
- `docs/design/MemoX Design System/components/navigation/MxSearchDock.jsx` — class→CSS mapping.
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxSearchDock**.
- `lib/core/theme/` — tokens + `MxTheme` extension.

## Output

- `lib/presentation/shared/composites/mx_search_dock.dart`
- `test/presentation/shared/composites/mx_search_dock_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/k08`.
...
- Kit name + base class are **frozen contract** — keep `MxSearchDock` + variant identifiers aligned.
