...
>
> Size **M** · Deps **DT.4** · Branch `build/v3`


## Goal

Query correctness, migration round-trips, seed load against an in-memory DB.
...
2. Push `build/v3`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `V.3` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.