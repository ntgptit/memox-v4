# V.5 — Prune-rule review on kit change
...
>
> Size **S** · Deps **V.4** · Branch `build/v5`


## Goal

When the kit changes, re-run the generator and review the pruned-token log.
...
- Re-include motion tokens if the app starts animating.
...
2. Push `build/v5`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
