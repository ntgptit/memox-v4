...
> **Loop task** (reusable component). Self-contained — execute fully in one iteration, then tick `C1.09` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S** · Deps **Phase F** · Branch `build/c109`


## Goal

Build the Flutter widget **MxIconTile** mirroring the kit component, driven entirely by design tokens + the theme (Tier 1).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/surfaces/MxIconTile.d.ts` — the typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/surfaces/MxIconTile.prompt.md` — the one-paragraph intent + JSX usage examples.
- `docs/design/MemoX Design System/components/surfaces/MxIconTile.jsx` — the class→CSS mapping (which base class + modifiers).
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxIconTile** (find its selector block).
- `lib/core/theme/` — the tokens + `MxTheme` extension to consume.

## Output

- `lib/presentation/shared/widgets/surfaces/mx_icon_tile.dart` — the widget.
- `test/presentation/shared/widgets/mxicontile_test.dart` — its test.

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/c109`.
...
- The kit name + base class are **frozen contract** — keep the Dart name `MxIconTile` and its variant identifiers aligned to the kit.
...
2. Push `build/c109`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
   > When pushing from an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Tick `C1.09` → `[x]` in `docs/agent/build/README.md`, small commit.