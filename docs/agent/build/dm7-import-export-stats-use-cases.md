# DM.7 — Import/export + stats use cases

> **Loop task** (domain (BE core)). Self-contained — execute fully in one iteration, then tick `DM.7` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **DM.3** · Branch `build/dm7`


## Goal

Parse/emit deck formats; compute statistics + heatmap.

## Inputs — read first

- `docs/business/import-export/import-export.md`
- `docs/business/statistics/statistics.md`

## Output

- `lib/domain/usecases/{io,stats}/*.dart`
...
2. Push `build/dm7`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `DM.7` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.