...
> **Loop task** (screen). Self-contained — execute fully in one iteration, then tick `S.01` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **L** · Deps **Phase K,H + DM.5** · Branch `build/s01`


## Goal

Build the **dashboard** screen + its 4 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.5** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/Dashboard.jsx` — screen composition (components, states, state machine).
- Feature-local components (build here):
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/components/ContinueCard.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/components/GoalCard.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/components/StreakCard.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/components/TodaySummary.jsx`
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/dashboard.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/dashboard--*--{light,dark}.png` — visual reference per state.
...
- `lib/presentation/features/dashboard/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
- `lib/presentation/features/dashboard/widgets/*.dart` — the 4 feature-local component(s)
- `test/presentation/features/dashboard/*_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/s01`.
2. Read `Dashboard.jsx` → enumerate **states** (screen + `specs/dashboard.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s01`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.01` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.