...
>
> Size **L** · Deps **Phase C1** · Branch `build/s01`


## Goal

Build the **dashboard** screen + its 4 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/dashboard/Dashboard.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/dashboard.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/dashboard--*--{light,dark}.png` — the visual reference for **every** state.
...
2. Read `Dashboard.jsx` → enumerate the **states** (from the screen + `specs/dashboard.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s01`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
