...
>
> Size **M** · Deps **Phase C1** · Branch `build/s04`


## Goal

Build the **search** screen + its 2 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/search/Search.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/search.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/search--*--{light,dark}.png` — the visual reference for **every** state.
...
2. Read `Search.jsx` → enumerate the **states** (from the screen + `specs/search.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s04`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
