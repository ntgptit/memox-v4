...
> **Loop task** (screen). Self-contained — execute fully in one iteration, then tick `S.04` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **Phase K,H + DM.6** · Branch `build/s04`


## Goal

Build the **search** screen + its 2 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.6** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/search/Search.jsx` — screen composition (components, states, state machine).
- Feature-local components (build here):
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/search/components/Chips.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/search/components/ResultRow.jsx`
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/search.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/search--*--{light,dark}.png` — visual reference per state.
- Shared widgets in `lib/presentation/shared/{primitives,composites}/`
- Domain use cases: `lib/domain/usecases/` (**DM.6**)

## Output

- `lib/presentation/features/search/screens/search_screen.dart`
- `lib/presentation/features/search/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
- `lib/presentation/features/search/widgets/*.dart` — the 2 feature-local component(s)
- `test/presentation/features/search/*_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/s04`.
2. Read `Search.jsx` → enumerate **states** (screen + `specs/search.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s04`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.04` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.