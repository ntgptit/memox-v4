...
- Wire the central error capture here: `FlutterError.onError` + `PlatformDispatcher.onError` + the guarded zone → route to the logger + an error-reporting hook (dev/monitoring side of AGENTS.md error handling).
...
3. Mark done: append `I.4` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.