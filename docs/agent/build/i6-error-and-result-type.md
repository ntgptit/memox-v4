# I.6 — Error and Result type

> **Loop task** (infrastructure). Self-contained — execute fully in one iteration, then tick `I.6` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S** · Deps **I.3** · Branch `build/i6`


## Goal

Failure hierarchy + Result<T> + guard helpers used across domain/data.
...
1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/i6`.
...
- Sealed Failure types; Result/Either; a runGuarded that maps exceptions -> Failure (never swallow — always map + log).
- Define the Failure → l10n-message mapper contract (end-user side) + the logger/reporting seam (dev side) so every error can be both shown and observed. No `print`.
...
2. Push `build/i6`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `I.6` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.