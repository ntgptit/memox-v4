# P.07 — MxSegmentedControl

> **Loop task** (primitive component). Self-contained — execute fully in one iteration, then tick `P.07` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S–M** · Deps **Phase T** · Branch `build/p07`


## Goal

Build the Flutter widget **MxSegmentedControl** mirroring the kit component, driven entirely by design tokens + `MxTheme` (primitive layer).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/core/MxSegmentedControl.d.ts` — typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/core/MxSegmentedControl.prompt.md` — intent + JSX usage examples.
- `docs/design/MemoX Design System/components/core/MxSegmentedControl.jsx` — class→CSS mapping.
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxSegmentedControl**.
- `lib/core/theme/` — tokens + `MxTheme` extension.

## Output

- `lib/presentation/shared/primitives/mx_segmented_control.dart`
- `test/presentation/shared/primitives/mx_segmented_control_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/p07`.
...
- Kit name + base class are **frozen contract** — keep `MxSegmentedControl` + variant identifiers aligned.
...
2. Push `build/p07`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `P.07` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.