# T.5 — Verification harness (goldens)

> **Loop task** (theme / UI foundation). Self-contained — execute fully in one iteration, then tick `T.5` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **T.2** · Branch `build/t5`


## Goal

Golden-test infra (font loading) + gate wiring.
...
1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/t5`.
...
2. Push `build/t5`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `T.5` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.