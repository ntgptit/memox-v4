# DT.2 — Migrations & versioning

> **Loop task** (data (Drift)). Self-contained — execute fully in one iteration, then tick `DT.2` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **DT.1** · Branch `build/dt2`


## Goal

Schema versioning + migration strategy + schema round-trip tests.
...
- Never edit a shipped schema in place — migrate forward (R3).
...
2. Push `build/dt2`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `DT.2` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.