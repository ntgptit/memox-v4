...
>
> Size **L** · Deps **Phase K,H + DM.5** · Branch `build/s20`


## Goal

Build the **study-session** screen + its 9 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.5** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/study-session/StudySession.jsx` — screen composition (components, states, state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/study-session.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/study-session--*--{light,dark}.png` — visual reference per state.
- Shared widgets in `lib/presentation/shared/{primitives,composites}/`
- Domain use cases: `lib/domain/usecases/` (**DM.5**)

## Output

- `lib/presentation/features/study-session/screens/study_session_screen.dart`
- `lib/presentation/features/study-session/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
...
2. Read `StudySession.jsx` → enumerate **states** (screen + `specs/study-session.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s20`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.20` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.