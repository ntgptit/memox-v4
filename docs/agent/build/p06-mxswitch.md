...
> **Loop task** (primitive component). Self-contained — execute fully in one iteration, then tick `P.06` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S–M** · Deps **Phase T** · Branch `build/p06`


## Goal

Build the Flutter widget **MxSwitch** mirroring the kit component, driven entirely by design tokens + `MxTheme` (primitive layer).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/core/MxSwitch.d.ts` — typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/core/MxSwitch.prompt.md` — intent + JSX usage examples.
- `docs/design/MemoX Design System/components/core/MxSwitch.jsx` — class→CSS mapping.
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxSwitch**.
- `lib/core/theme/` — tokens + `MxTheme` extension.

## Output

- `lib/presentation/shared/primitives/mx_switch.dart`
- `test/presentation/shared/primitives/mx_switch_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/p06`.
...
- Kit name + base class are **frozen contract** — keep `MxSwitch` + variant identifiers aligned.
...
2. Push `build/p06`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `P.06` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.