...
>
> Size **M** · Deps **Phase C1** · Branch `build/s14`


## Goal

Build the **game-matching** screen + its 1 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/game-matching/GameMatching.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/game-matching.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/game-matching--*--{light,dark}.png` — the visual reference for **every** state.
- Tier-1 widgets in `lib/presentation/shared/widgets/` + tokens/theme.

## Output

- `lib/presentation/features/game-matching/game_matching_screen.dart`
...
2. Read `GameMatching.jsx` → enumerate the **states** (from the screen + `specs/game-matching.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s14`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
