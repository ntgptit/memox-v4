...
> **Loop task** (reusable component). Self-contained — execute fully in one iteration, then tick `C1.13` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **Phase F** · Branch `build/c113`


## Goal

Build the Flutter widget **MxBottomNav** mirroring the kit component, driven entirely by design tokens + the theme (Tier 1).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/navigation/MxBottomNav.d.ts` — the typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/navigation/MxBottomNav.prompt.md` — the one-paragraph intent + JSX usage examples.
- `docs/design/MemoX Design System/components/navigation/MxBottomNav.jsx` — the class→CSS mapping (which base class + modifiers).
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxBottomNav** (find its selector block).
- `lib/core/theme/` — the tokens + `MxTheme` extension to consume.

## Output

- `lib/presentation/shared/widgets/navigation/mx_bottom_nav.dart` — the widget.
- `test/presentation/shared/widgets/mxbottomnav_test.dart` — its test.

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/c113`.
...
- The kit name + base class are **frozen contract** — keep the Dart name `MxBottomNav` and its variant identifiers aligned to the kit.
...
2. Push `build/c113`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
   > When pushing from an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Tick `C1.13` → `[x]` in `docs/agent/build/README.md`, small commit.