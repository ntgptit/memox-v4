...
> **Loop task** (domain (BE core)). Self-contained — execute fully in one iteration, then tick `DM.2` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **DM.1** · Branch `build/dm2`


## Goal

Immutable entities: Deck (nested), Card, LanguagePair, ReviewLog, StudySession, Goal, Streak, DeckStats.
...
- Deck nesting = parent/children. Keep domain entities separate from Drift row models.
...
2. Push `build/dm2`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `DM.2` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.