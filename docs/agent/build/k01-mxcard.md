...
> **Loop task** (composite component). Self-contained — execute fully in one iteration, then tick `K.01` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S–L** · Deps **Phase T** · Branch `build/k01`


## Goal

Build the Flutter widget **MxCard** mirroring the kit component, driven entirely by design tokens + `MxTheme` (composite layer).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/surfaces/MxCard.d.ts` — typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/surfaces/MxCard.prompt.md` — intent + JSX usage examples.
- `docs/design/MemoX Design System/components/surfaces/MxCard.jsx` — class→CSS mapping.
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxCard**.
- `lib/core/theme/` — tokens + `MxTheme` extension.

## Output

- `lib/presentation/shared/composites/mx_card.dart`
- `test/presentation/shared/composites/mx_card_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/k01`.
...
- Kit name + base class are **frozen contract** — keep `MxCard` + variant identifiers aligned.
...
2. Push `build/k01`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `K.01` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.