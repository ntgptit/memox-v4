# DM.6 — Library use cases

> **Loop task** (domain (BE core)). Self-contained — execute fully in one iteration, then tick `DM.6` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **DM.3** · Branch `build/dm6`


## Goal

Deck CRUD (nested move/rename/delete), card CRUD, soft-dup detection, term+meaning search.

## Inputs — read first

- `docs/design/MemoX Design System/ui_kits/memox-app/specs/{library,deck-detail,search,flashcard-editor}.md`

## Output

- `lib/domain/usecases/library/*.dart`
...
2. Push `build/dm6`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Tick `DM.6` → `[x]` in `docs/agent/build/README.md`, small commit.