# V.2 — Accessibility pass

> **Loop task** (verification). Self-contained — execute fully in one iteration, then tick `V.2` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **Phase C1** · Branch `build/v2`


## Goal

Touch targets, contrast, semantics labels to WCAG AA on interactive widgets.

## Inputs — read first

- `MxSpacing.minTouchTarget`
- `MxColors on/soft pairs`
...
1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/v2`.
...
- Every interactive widget ≥ 48dp; semantics labels present.
...
2. Push `build/v2`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
   > When pushing from an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Tick `V.2` → `[x]` in `docs/agent/build/README.md`, small commit.