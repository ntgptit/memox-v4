...
> **Loop task** (screen). Self-contained — execute fully in one iteration, then tick `S.22` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **L** · Deps **Phase C1** · Branch `build/s22`
...
Build the **account-sync** screen + its 3 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/account-sync/AccountSync.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/account-sync.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/account-sync--*--{light,dark}.png` — the visual reference for **every** state.
- Tier-1 widgets in `lib/presentation/shared/widgets/` + tokens/theme.

## Output

- `lib/presentation/features/account-sync/account_sync_screen.dart`
- `lib/presentation/features/account-sync/widgets/*.dart` — the 3 feature-local component(s).
- `test/presentation/features/account-sync/*_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/s22`.
2. Read `AccountSync.jsx` → enumerate the **states** (from the screen + `specs/account-sync.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s22`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
   > When pushing from an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Tick `S.22` → `[x]` in `docs/agent/build/README.md`, small commit.