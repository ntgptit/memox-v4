...
Build the **account-sync** screen + its 3 feature-local component(s), composed from the shared `Mx*` widgets, rendering local UI state, matching the kit for every state.
...
- `lib/presentation/features/account-sync/screens/account_sync_screen.dart`
- `lib/presentation/features/account-sync/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
...
2. Read `AccountSync.jsx` → enumerate **states** (screen + `specs/account-sync.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s22`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.22` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.