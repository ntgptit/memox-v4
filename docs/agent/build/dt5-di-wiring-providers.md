# DT.5 — DI wiring (providers)

> **Loop task** (data (Drift)). Self-contained — execute fully in one iteration, then tick `DT.5` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **DT.4** · Branch `build/dt5`


## Goal

@riverpod providers exposing repositories + use cases; swap in-memory fakes for Drift.
...
- This is the seam that flips screens from fakes to real data.
...
2. Push `build/dt5`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `DT.5` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.