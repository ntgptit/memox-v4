...
> **Loop task** (screen). Self-contained — execute fully in one iteration, then tick `S.17` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **Phase K,H + DM.5** · Branch `build/s17`


## Goal

Build the **game-typing** screen + its 2 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.5** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/game-typing/GameTyping.jsx` — screen composition (components, states, state machine).
- Feature-local components (build here):
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/game-typing/components/CharCompare.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/game-typing/components/InputBox.jsx`
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/game-typing.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/game-typing--*--{light,dark}.png` — visual reference per state.
- Shared widgets in `lib/presentation/shared/{primitives,composites}/`
- Domain use cases: `lib/domain/usecases/` (**DM.5**)

## Output

- `lib/presentation/features/game-typing/screens/game_typing_screen.dart`
- `lib/presentation/features/game-typing/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
- `lib/presentation/features/game-typing/widgets/*.dart` — the 2 feature-local component(s)
- `test/presentation/features/game-typing/*_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/s17`.
2. Read `GameTyping.jsx` → enumerate **states** (screen + `specs/game-typing.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s17`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.17` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.