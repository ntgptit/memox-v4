...
> **Loop task** (primitive component). Self-contained — execute fully in one iteration, then tick `P.01` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S–M** · Deps **Phase T** · Branch `build/p01`


## Goal

Build the Flutter widget **MxButton** mirroring the kit component, driven entirely by design tokens + `MxTheme` (primitive layer).

## Inputs — READ ALL IN FULL (do not infer)

- `docs/design/MemoX Design System/components/core/MxButton.d.ts` — typed prop contract (variants, sizes, flags).
- `docs/design/MemoX Design System/components/core/MxButton.prompt.md` — intent + JSX usage examples.
- `docs/design/MemoX Design System/components/core/MxButton.jsx` — class→CSS mapping.
- `docs/design/MemoX Design System/components.css` — the base class + modifier styling for **MxButton**.
- `lib/core/theme/` — tokens + `MxTheme` extension.

## Output

- `lib/presentation/shared/primitives/mx_button.dart`
- `test/presentation/shared/primitives/mx_button_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/p01`.
...
- Kit name + base class are **frozen contract** — keep `MxButton` + variant identifiers aligned.
...
2. Push `build/p01`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `P.01` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.