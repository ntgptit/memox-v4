...
> **Loop task** (screen). Self-contained — execute fully in one iteration, then tick `S.08` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **Phase K,H + DM.8** · Branch `build/s08`


## Goal

Build the **theme** screen + its 2 feature-local component(s), composed from the shared `Mx*` widgets, rendering **DM.8** use-case state via `@riverpod` providers, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/theme/Theme.jsx` — screen composition (components, states, state machine).
- Feature-local components (build here):
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/theme/components/AccentPicker.jsx`
  - `docs/design/MemoX Design System/ui_kits/memox-app/_features/theme/components/PreviewCard.jsx`
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/theme.md` — contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/theme--*--{light,dark}.png` — visual reference per state.
- Shared widgets in `lib/presentation/shared/{primitives,composites}/`
- Domain use cases: `lib/domain/usecases/` (**DM.8**)

## Output

- `lib/presentation/features/theme/screens/theme_screen.dart`
- `lib/presentation/features/theme/providers/*.dart` — `@riverpod` notifier(s) (own mutation; call use cases)
- `lib/presentation/features/theme/widgets/*.dart` — the 2 feature-local component(s)
- `test/presentation/features/theme/*_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/s08`.
2. Read `Theme.jsx` → enumerate **states** (screen + `specs/theme.md` + `shots/` filenames) and the components each renders.
...
2. Push `build/s08`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `S.08` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.