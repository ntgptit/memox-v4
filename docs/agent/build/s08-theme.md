...
>
> Size **M** · Deps **Phase C1** · Branch `build/s08`


## Goal

Build the **theme** screen + its 2 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/theme/Theme.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/theme.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/theme--*--{light,dark}.png` — the visual reference for **every** state.
- Tier-1 widgets in `lib/presentation/shared/widgets/` + tokens/theme.

## Output

- `lib/presentation/features/theme/theme_screen.dart`
...
2. Read `Theme.jsx` → enumerate the **states** (from the screen + `specs/theme.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s08`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
