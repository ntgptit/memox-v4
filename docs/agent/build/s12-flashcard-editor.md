...
>
> Size **M** · Deps **Phase C1** · Branch `build/s12`


## Goal

Build the **flashcard-editor** screen + its 2 feature-local component(s), composed from the Tier-1 `Mx*` widgets, matching the kit for every state.

## Inputs — READ ALL IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/_features/flashcard-editor/FlashcardEditor.jsx` — the screen composition (which components, which states, the state machine).
...
- `docs/design/MemoX Design System/ui_kits/memox-app/specs/flashcard-editor.md` — the contract (states, copy, behaviour).
- `docs/design/MemoX Design System/ui_kits/memox-app/shots/flashcard-editor--*--{light,dark}.png` — the visual reference for **every** state.
- Tier-1 widgets in `lib/presentation/shared/widgets/` + tokens/theme.

## Output

- `lib/presentation/features/flashcard-editor/flashcard_editor_screen.dart`
...
2. Read `FlashcardEditor.jsx` → enumerate the **states** (from the screen + `specs/flashcard-editor.md` + the `shots/` filenames) and which components each state renders.
...
2. Push `build/s12`; open a PR; merge to main (`--merge --delete-branch`); `git checkout main && git pull`.
