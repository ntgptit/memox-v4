...
> **Loop task** (shared helper (primitive)). Self-contained — execute fully in one iteration, then tick `H.07` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S** · Deps **Phase T** · Branch `build/h07`


## Goal

Build **MxChoiceOption** — the Flutter port of the kit's `ChoiceOption` helper — as a reusable primitive, token-driven. These helpers are reused across many screens; building them shared avoids per-screen re-derivation.

## Inputs — READ IN FULL

- `docs/design/MemoX Design System/ui_kits/memox-app/kit-helpers.jsx` — find `function ChoiceOption` (the exact styling + tokens).
- `docs/design/MemoX Design System/components.css` — any `.mxg-*` class it uses.
- `lib/core/theme/` — tokens + `MxTheme`.

## Output

- `lib/presentation/shared/primitives/mx_choice_option.dart`
- `test/presentation/shared/primitives/mx_choice_option_test.dart`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/h07`.
2. Read `function ChoiceOption` in `kit-helpers.jsx` → derive props + token-based styling. **No raw `Color(0x..)`/px.**
3. answer option; tone default/correct/wrong with check/cancel icon. Game MC/choice.
...
- Name it `MxChoiceOption` (Mx-prefixed shared widget). Strings from ARB.
...
2. Push `build/h07`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `H.07` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.