# V.6 — Responsive check

> **Loop task** (verification). Self-contained — execute fully in one iteration, then tick `V.6` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **M** · Deps **T.6** · Branch `build/v6`


## Goal

Phone widths correct; large screens graceful.
...
- `responsive tests/notes`

## Steps

1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/v6`.
...
2. Push `build/v6`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `V.6` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.