# I.2 — Lint and format config

> **Loop task** (infrastructure). Self-contained — execute fully in one iteration, then tick `I.2` in `docs/agent/build/README.md`. One task per iteration.
>
> Size **S** · Deps **I.1** · Branch `build/i2`


## Goal

Strict analyzer config enforcing the layer contracts.

## Inputs — read first

- `analysis_options.yaml`
- `flutter-full-app architecture rules`
...
1. **Baseline**: `git checkout main && git pull`, `git checkout -b build/i2`.
...
- flutter_lints + riverpod_lint/custom_lint.
- Where feasible, forbid feature UI importing data/ or dart:io.
...
2. Push `build/i2`; open a PR; merge to main; `git checkout main && git pull`.
   > From an agent session without a design-authorized TTY, prefix: `MEMOX_SKIP_DESIGN_SYNC=1 git push …`.
3. Mark done: append `I.2` to `docs/agent/build/DONE.txt`, run `node tool/design/gen_task_prompts.mjs` (renders `[x]` in the queue), commit.