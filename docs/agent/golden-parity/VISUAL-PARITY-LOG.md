...
| 11 | dashboard · empty | kit shot = old not-studied; Flutter = new onboarding | FIX | **DONE (loop-2b)** — re-exported the shot from the kit source via a new headless-Chrome exporter (`tool/design/export_kit_shot.mjs`); shot now shows onboarding. Residual ~22% is the usual greeting/date + cross-renderer AA noise |
| 12 | study-session · * | Korean term renders as tofu boxes in goldens | FIX | **DONE (loop-2b)** — merged a tiny OFL Noto Sans KR subset into Plus Jakarta per weight (test/fonts/pjs-cjk-*.ttf); 학교 renders as Korean. Regen: `python tool/design/build_cjk_test_font.py` |
