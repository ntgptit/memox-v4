...
**Ghi chú giao diện (parity — divergence có chủ đích):** ở màn library home, ba control
tìm-kiếm / sắp-xếp / tạo-bộ-thẻ được đặt trong **thanh toolbar cố định** và render ở **mọi
trạng thái** của màn (loaded / empty / error / loading). Design-kit lại đặt search+sort trong
`library/context` (phần body chỉ hiện khi có bộ thẻ) và chỉ đưa nút tạo vào state `loaded`;
FE giữ cả ba luôn hiển thị để thao tác tổ chức thư viện luôn sẵn sàng (kể cả khi rỗng/lỗi,
người học vẫn tạo được bộ thẻ đầu tiên hoặc thử lại). Đây là khác biệt FE↔kit đã ghi nhận
tại `tool/parity/intent-ledger.json` (`library/search-btn`, `library/sort-btn` =
`exceptionKind: behavior`; `library/create` = `styleExempt` cho hình thức + `behavior` cho
việc luôn hiển thị). Ba node vẫn được key nên gate state-composition **loại chúng khỏi
universe** (không phân biệt state) — xem `tool/parity/contracts/library.states.json`.
