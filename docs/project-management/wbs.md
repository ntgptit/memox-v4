...
- 1d1e5072 · 2026-06-28 · W7 · tìm kiếm khớp đa từ-khoá AND (tách token theo khoảng trắng; mỗi token khớp term hoặc nghĩa) trong SearchDao — cải tiến recall không đổi schema; FTS5 đánh giá & hoãn đúng theo spec (LIKE tới khi perf cần); test multi-token + docs global-search (BR-1/AC-4/status) + D-019 (sửa luôn ref cũ card_meaning.text→.content)
