...
| props parity: every kit component's `.d.ts` API contract vs its Flutter constructor — authored + gated + wiring-guarded | 83 `.d.ts` contracts (68 authored: `_features/*/components/*.d.ts` + `_shared/*.d.ts`; 15 pre-existing shared) · checker `tool/parity/props_check.mjs` (`--strict` blocking in `tool/verify/run.mjs`) · config `tool/parity/props_map.json` · typed-exception ledger `props-parity.exceptions.json` (192, closed reason set) · doc `tool/parity/README.md` | `test/tooling/props_check_gate_test.dart` (run.mjs invokes `--strict` + calls it · CI runs the gate · checker+map+exceptions+README exist · REASONS enforced · ledger valid) | C.01–C.23 · Z.0–Z.2 | #149–177 |
| visual parity: NEW spec/shot exporter (`tool/ui_kit_shots/` rebuilt from scratch — the pre-reset tool is reference-only at `git f702ecb^`) + freshness/completeness gates + drift watch — **planned, not built** | plan `docs/agent/visual-parity/WBS.md` (phases VP.0–VP.6, decisions D-VP-1…5) · outputs: `ui_kits/memox-app/{shots,specs}` regenerated (current ones are STALE since the repo reset — dashboard diverged in PRs #207/#208/#210) | per WBS: wiring-guard test + `check:fresh`/`check:complete` in `tool/verify/run.mjs` + CI export job | VP.0–VP.5 | — |
