...
function Body({ state }) {
  ...
  if (state === 'stage4-recall') {
    return (
      <React.Fragment>
        <PromptCard term="친구" />
        <MxCard style={{ alignItems: 'center', minHeight: 90, justifyContent: 'center', color: 'var(--memox-text-tertiary)' }}>Recall the meaning, then tap “Show”</MxCard>
  ...
  if (state === 'stage5-typing') {
    return (
      <React.Fragment>
        ...
        <div style={{ border: '1px solid var(--memox-divider)', background: 'var(--memox-surface)', borderRadius: 'var(--memox-radius-control)', padding: 16, minHeight: 56, color: 'var(--memox-text-tertiary)', fontWeight: 600 }}>Type the Korean word…</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--memox-space-3)' }}>
